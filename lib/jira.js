...
async function buildTrendDataForRange(credentials, epicsWithChildren, range = {}) {
  ...
  const expandedChildIssues = await expandIssuesWithSubtasks(
    credentials,
    childIssues
  );

  const childWorklogs = await Promise.all(
    expandedChildIssues.map(async (issue) =>
...
async function calculateTimeSpentLastWeek(credentials, epicsWithChildren, referenceDate = new Date()) {
  ...
  const childIssues = await expandIssuesWithSubtasks(credentials, directChildIssues);
  ...
  const breakdown = childIssues
    .map((issue, index) => {
      const parentEpic = epicsWithChildren.find((epic) =>
        (epic.childIssues || []).some(
          (child) =>
            child.key === issue.key ||
            (child.fields?.subtasks || []).some((subtask) => subtask.key === issue.key)
        )
      );
...
async function getFilteredEpicsWithChildren(credentials, projectKey, filters = {}) {
  ...
  const epicsWithChildren = childLookups.map(({ epic, childLookup }) => ({
    ...epic,
