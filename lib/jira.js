...
async function calculateTimeSpentLastWeek(credentials, epicsWithChildren, referenceDate = new Date()) {
  const referenceUtcDay = startOfUtcDay(referenceDate);
  const currentWeekStart = addUtcDays(
    referenceUtcDay,
    -((referenceUtcDay.getUTCDay() + 6) % 7)
  );
  const rangeStart = addUtcDays(currentWeekStart, -7);
  const rangeEnd = new Date(currentWeekStart.getTime() - 1);
  const childIssues = [
    ...new Map(
      epicsWithChildren
        .flatMap((epic) => epic.childIssues || [])
  ...
  return roundHours(
    childWorklogs.reduce(
      (sum, worklogs) => sum + sumWorklogHoursInRange(worklogs, rangeStart, rangeEnd),
      0
    )
  );
}
...
async function handleReport(req, res) {
  try {
    ...
    sendJson(res, 200, {
      ...
      summary: {
        ...summarize(rows),
        timeSpentLastWeekPrefill: timeSpentLastWeek,
      },
      debug: {
        projectKey,
