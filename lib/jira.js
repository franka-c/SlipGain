...
function buildBucketDates(startDate, endDate, granularity) {
  ...
  if (buckets.length === 0 || !isSameUtcDay(buckets[buckets.length - 1], safeEnd)) {
    buckets.push(safeEnd);
  }
    ...
    .flatMap((history) =>
      (history.items || [])
        ...
        .map((item) => ({
          ...
          fromValue: valueParser(item.from),
          toValue: valueParser(item.to),
        }))
    )
...
function getFieldValueAtDate(issueCreatedAt, currentValue, events, targetDate) {
  ...
  let value = roundHours(currentValue);

  for (let index = events.length - 1; index >= 0; index -= 1) {
    const event = events[index];
    if (event.changedAt > targetDate) {
      value = roundHours(event.fromValue);
      continue;
    }
...
function buildTrendSeriesForGranularity(epics, epicChangelogs, worklogEntries, granularity) {
  const startDate = getEarliestRelevantDate(epics, epicChangelogs, worklogEntries);
...
async function buildTrendData(credentials, epicsWithChildren) {
  ...
  const worklogEntries = buildWorklogEntries(childWorklogs.flat());
...
async function handleReport(req, res) {
  try {
    ...
    for (const epic of filteredEpics) {
      ...
      epicsWithChildren.push(epicWithChildren);
      rows.push(
        calculateRow(
          epicWithChildren,
          REMAINING_ESTIMATE_FIELD_ID
    ...
    const trends = await buildTrendData(credentials, epicsWithChildren);
