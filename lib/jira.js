...
async function fetchIssueChangelog(credentials, issueKey, bypassCache = false) {
  ...
  if (!bypassCache && cachedEntry?.value && cachedEntry.expiresAt > Date.now()) {
    return cachedEntry.value;
  }

  if (!bypassCache && cachedEntry?.promise) {
...
async function fetchIssueWorklogs(credentials, issueKey, bypassCache = false) {
...
function getResolvedEpicIssues(epic) {
  const resolvedIssues = epic.resolvedChildIssues || epic.childIssues || [];
  const epicTimeSpent = toHours(epic.fields?.timespent);

  if (epicTimeSpent <= 0) {
    return resolvedIssues;
  }

  return [epic, ...resolvedIssues];
}
...
async function buildTrendDataForRange(
  credentials,
  epicsWithChildren,
  range = {},
  bypassCache = false
  ...
  const epicChangelogEntries = await mapWithConcurrency(
    ...
    async (epic) => [
      epic.key,
      await fetchIssueChangelog(credentials, epic.key, bypassCache),
    ]
  );
  ...
  const childWorklogs = await mapWithConcurrency(
    ...
    async (issue) => fetchIssueWorklogs(credentials, issue.key, bypassCache)
  );
...
function sumWorklogHoursUpToDate(worklogs, targetDate) {
  return roundHours(
    buildWorklogEntries(worklogs)
      .filter((entry) => entry.startedAt <= targetDate)
      .reduce((sum, entry) => sum + entry.hours, 0)
  );
}

function calculateSnapshotRow(epic, histories, worklogsByIssueKey, targetDate) {
  const createdAt = parseDate(epic.fields?.created);
  const originalEvents = extractFieldChangeEvents(
    histories,
    matchesOriginalEstimateChange,
    parseEstimateSecondsToHours
  );
  const remainingEvents = extractFieldChangeEvents(
    histories,
    matchesRemainingEstimateChange,
    parseNumericHours
  );
  const originalEstimate = getFieldValueAtDate(
    createdAt,
    toHours(epic.fields?.timeoriginalestimate),
    originalEvents,
    targetDate
  );
  const remainingEstimate = getFieldValueAtDate(
    createdAt,
    parseNumericHours(epic.fields?.[REMAINING_ESTIMATE_FIELD_ID]),
    remainingEvents,
    targetDate
  );
  const timeSpent = getResolvedEpicIssues(epic).reduce((sum, issue) => {
    const worklogs = worklogsByIssueKey.get(issue.key) || [];
    return sum + sumWorklogHoursUpToDate(worklogs, targetDate);
  }, 0);
  const slipGain = Number(
    (originalEstimate - remainingEstimate - timeSpent).toFixed(2)
  );
  ...
  return {
    issueKey: epic.key,
    summary: epic.fields?.summary || "",
    originalEstimate: roundHours(originalEstimate),
    remainingEstimate: roundHours(remainingEstimate),
    timeSpent: Number(timeSpent.toFixed(2)),
    slipGain,
    progress,
  };
...
async function buildSnapshotRows(
  ...
  const parsedSnapshot = parseDate(snapshotDate);
  if (!parsedSnapshot) {
    throw new Error("Snapshot date is invalid. Use a YYYY-MM-DD date.");
  }
  const targetDate = endOfUtcDay(parsedSnapshot);

  // Epics that did not exist yet at the snapshot date are excluded entirely.
  const eligibleEpics = epicsWithChildren.filter((epic) => {
    const createdAt = parseDate(epic.fields?.created);
  ...
  const epicChangelogs = new Map(changelogEntries);

  const childIssues = getScopedChildIssues(eligibleEpics);
  const worklogEntries = await mapWithConcurrency(
    childIssues,
    ISSUE_WORKLOG_FETCH_CONCURRENCY,
    async (issue) => [
      issue.key,
      await fetchIssueWorklogs(credentials, issue.key, bypassCache),
    ]
  );
  const worklogsByIssueKey = new Map(worklogEntries);

  return eligibleEpics.map((epic) =>
    calculateSnapshotRow(
      epic,
      epicChangelogs.get(epic.key) || [],
      worklogsByIssueKey,
      targetDate
    )
  );
...
async function calculateTimeSpentLastWeek(
  credentials,
  epicsWithChildren,
...
async function getFilteredEpicsWithChildren(
  credentials,
  projectKey,
  filters = {},
  bypassCache = false
...
async function handleReport(req, res) {
  try {
    ...
    const credentials = getCredentials(body);
    const bypassCache = Boolean(body.refresh);
    const snapshotDate = normalizeName(body.snapshotDate);
    const { epicJql, epics, filteredEpics, epicsWithChildren, childLookupDebug } =
      await getFilteredEpicsWithChildren(
        credentials,
        projectKey,
        body.filters,
        bypassCache
...
async function handleLastWeekTime(req, res) {
  try {
    ...
    const credentials = getCredentials(body);
    const bypassCache = Boolean(body.refresh);
    const referenceDate = parseDate(body.snapshotDate) || new Date();
...
async function handleTrends(req, res) {
  try {
    ...
    if (!bypassCache && cachedEntry && cachedEntry.expiresAt > Date.now()) {
