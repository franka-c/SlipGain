...
async function buildChildLookupTrace(credentials, projectKey, issueKey, epicsWithChildren, childLookupDebug) {
  ...
  const issueResults = await searchIssues(
    ...
    ["summary", "issuetype", "parent", "customfield_10014"]
  );
  const issue = issueResults[0] || null;
  const includedEpic = epicsWithChildren.find((epic) =>
    (epic.childIssues || []).some((child) => child.key === normalizedIssueKey)
  );
  const parentKey = issue?.fields?.parent?.key || null;
  const directRelationshipChecks = [];

  if (parentKey) {
    const directQueries = [
      `key = "${normalizedIssueKey}" AND parent = "${parentKey}"`,
      `key = "${normalizedIssueKey}" AND "Epic Link" = "${parentKey}"`,
      `key = "${normalizedIssueKey}" AND parentEpic = "${parentKey}"`,
    ];

    for (const jql of directQueries) {
      try {
        const issues = await searchIssues(credentials, jql, ["summary", "issuetype"]);
        directRelationshipChecks.push({
          jql,
          issueCount: issues.length,
          matchedKeys: issues.map((result) => result.key),
          success: true,
        });
      } catch (error) {
        directRelationshipChecks.push({
  ...
  return {
    ...
    matchedIncludedEpic: includedEpic
      ? {
          epicKey: includedEpic.key,
          epicSummary: includedEpic.fields?.summary || "",
        }
      : null,
    directRelationshipChecks,
    childLookupAttempts: childLookupDebug
      .filter((entry) =>
        entry.attempts.some((attempt) =>
          typeof attempt.jql === "string" && attempt.jql.includes(normalizedIssueKey.split("-")[0])
        )
      )
      .map((entry) => ({
        ...
        attempts: entry.attempts.map((attempt) => ({
          ...attempt,
          containsIssue: Boolean(
            epicsWithChildren
              .find((epic) => epic.key === entry.epicKey)
              ?.childIssues?.some((child) => child.key === normalizedIssueKey)
          ),
        })),
      })),
  };
...
async function calculateTimeSpentLastWeek(credentials, epicsWithChildren, referenceDate = new Date()) {
  ...
  const childWorklogs = await Promise.all(
    childIssues.map(async (issue) => fetchIssueWorklogs(credentials, issue.key))
  );

  const breakdown = childIssues
    .map((issue, index) => {
      const parentEpic = epicsWithChildren.find((epic) =>
        (epic.childIssues || []).some((child) => child.key === issue.key)
      );
...
async function handleReport(req, res) {
  try {
    ...
    const timeSpentLastWeek = await calculateTimeSpentLastWeek(credentials, epicsWithChildren);
    const childLookupTrace = body.debugIssueKey
      ? await buildChildLookupTrace(
          credentials,
          projectKey,
          body.debugIssueKey,
          epicsWithChildren,
